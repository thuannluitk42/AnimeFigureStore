...
CREATE TABLE `categories` (
						 ...
						 `category_name` nvarchar(255)  NULL DEFAULT NULL,
                         `is_deleted` boolean,
                         `created_date` timestamp DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `orders_detail` (
                         `order_id` int(11),
						 `product_id` int(11),
						 `price` decimal comment 'gia luc tinh tien',
                         `amount` int ,
                         `total` timestamp DEFAULT CURRENT_TIMESTAMP,
