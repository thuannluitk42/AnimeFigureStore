...
public interface ProductRepository extends CrudRepository<Product, UUID>, JpaSpecificationExecutor<Product> {
    ...
    @Query("SELECT u from Product u where u.category.categoryId =:categoryId ORDER BY u.createdDate DESC limit 10")
    List<Product> find10ProductByCategoryId(int categoryId);

    Product getProductByProductId(Integer item);
}
