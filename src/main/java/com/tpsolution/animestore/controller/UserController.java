...
//    }
    ...
    @PostMapping(value ="/add-new-user", consumes = { "multipart/form-data" })
    public ResponseEntity<DataResponse> insertNewUser(@RequestPart("data") AddUserRequest request, @RequestParam("avatar") MultipartFile multipartFile) throws IOException {
        if (!multipartFile.isEmpty()) {
            String fileName = StringUtils.cleanPath(multipartFile.getOriginalFilename());
            request.setUrlImage(fileName);
        }
        return ResponseEntity.ok(userService.insertNewUser(request, multipartFile));
    }
...
//    @PostMapping(value = "/update-info-user")
//    public ResponseEntity<DataResponse> updateInfoUser(@RequestBody UpdateUserRequest request) throws IOException {
//        return ResponseEntity.ok(userService.updateUser(request));
//    }
    ...
    @PostMapping("/change-password-by-id")
    public ResponseEntity<DataResponse> changePasswordByUserId(@RequestBody ChangePWIdRequest request) {
        return ResponseEntity.ok(userService.changePasswordByUserId(request));
    }
